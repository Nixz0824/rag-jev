...
| 情况 | 条数 |
|---|---|
| 过滤后候选不足 2 条，Jev 无法介入 | 39 |
| Jev 改序 | **1**（改对 1、改错 0） |
| Jev 保留顺序 | 82（其中第 1 条本来就对 80、本来就错 2） |
...
- **不值**：元数据过滤已经把检索变简单，重排空间很小（83 条里只改 1 条）；解析层完全不需要它。
...
| 5 | 俗称缺失（船长/鸟皇/熊/猴子/诺手/铁男/冰女等） | 盲测单点题「船长 Q 蓝耗」「鸟皇基础生命值」 | 俗称表补 15 条 |
